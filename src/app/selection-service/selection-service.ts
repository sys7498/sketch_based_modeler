...
export class SelectionService {
     ...
     */
    protected updateMouseWorldPosition(
        clientX: number,
        clientY: number,
        depth: number
    ) {
        ...
        this._mouseRatioPosition = new Vector3(
            ...
            -((clientY - viewportDivRect.top) / viewportDivRect.height) * 2 + 1,
            depth
        );
        var vec = new Vector3(); // create once and reuse
        vec.copy(this._mouseRatioPosition);
        vec.unproject(this._sceneGraph.cameraSet.cameraSets['camera2d']);
        vec.setZ(10);
